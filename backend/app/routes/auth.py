...
def register(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    new_user = User(
        ...
        is_active=True,
        role="Admin" if user_data.email == "admin@telecomiq.com" else "Strategic Member"
    )
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
...
def request_otp(data: UserLogin, db: Session = Depends(get_db)):
    ...
    if not user:
        user = User(
            ...
            is_active=True,
            role="Admin" if data.email == "admin@telecomiq.com" else "Strategic Member"
        )
        db.add(user)
...
def google_auth(data: GoogleAuth, db: Session = Depends(get_db)):
    ...
    if not user:
        user = User(
            ...
            is_active=True,
            role="Admin" if email == "admin@telecomiq.com" else "Strategic Member"
        )
        db.add(user)
        db.commit()
        db.refresh(user)
